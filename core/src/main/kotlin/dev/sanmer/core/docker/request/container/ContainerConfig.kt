...
data class ContainerConfig(
    ...
    @SerialName("ExposedPorts") val exposedPorts: Map<String, ExposedPortValue> = emptyMap(),
    ...
    @SerialName("Healthcheck") val healthCheck: Health = Health(),
    @SerialName("ArgsEscaped") val argsEscaped: Boolean = false,
    @SerialName("Image") val image: String,
    @SerialName("Volumes") val volumes: Map<String, VolumeValue> = emptyMap(),
    @SerialName("WorkingDir") val workingDir: String,
    @SerialName("Entrypoint") val entryPoint: List<String>,
    ...
    @SerialName("NetworkingConfig") val networkingConfig: Networking
) {
    @Serializable
    @JvmInline
    ...
    data class Health(
        ...
        @SerialName("StartInterval") val startInterval: Long = 0
    )
    ...
    data class Networking(
        @SerialName("EndpointsConfig") val endpointsConfig: EndpointSettings
    ) {
        @Serializable
        data class EndpointSettings(
            @SerialName("IPAMConfig") val ipAMConfig: IPAMConfig = IPAMConfig(),
            ...
            @SerialName("GwPriority") val gwPriority: Int,
            ...
            @SerialName("DNSNames") val dnsNames: List<String> = emptyList()
        ) {
            @Serializable
