...
data class SystemDataUsage(
    @SerialName("LayersSize") val layersSize: Long,
    @SerialName("Images") val images: List<Image>,
    @SerialName("Containers") val containers: List<Container>,
    @SerialName("Volumes") val volumes: List<Volume>,
    @SerialName("BuildCache") val buildCache: List<BuildCache>
) {
    @Serializable
    data class BuildCache(
        ...
        @SerialName("InUse") val inUse: Boolean,
        @SerialName("Shared") val shared: Boolean,
        @SerialName("Size") val size: Long,
        @SerialName("CreatedAt") val createdAt: Instant,
        @SerialName("LastUsedAt") val lastUsedAt: Instant = Instant.fromEpochMilliseconds(0),
        @SerialName("UsageCount") val usageCount: Int
    )
}
