...
data class ImageLowLevel(
    ...
    @SerialName("Config") val config: ContainerLowLevel.Config,
    ...
    @SerialName("GraphDriver") val graphDriver: ContainerLowLevel.DriverData,
    @SerialName("RootFS") val rootFS: RootFS,
    @SerialName("Metadata") val metadata: Metadata
) {
    @Serializable
    data class RootFS(
        @SerialName("Type") val type: String,
        @SerialName("Layers") val layers: List<String>
    )

    @Serializable
    data class Metadata(
        @SerialName("LastTagTime") val lastTagTime: Instant = Instant.fromEpochMilliseconds(0)
    )
}
